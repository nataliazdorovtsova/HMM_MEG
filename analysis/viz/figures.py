...
def cognition_scatter(
    ...
    matlab/legacy/HMM_CognitiveBehaviouralAnalyses.m with one reusable function.
    """
    apply_style()
    ax = ax or plt.gca()
...
def transition_correlation_heatmap(
    ...
    **Descriptive only.** These 49 cells are not the confirmatory analysis:
    ...
    over transitions *into* each state (column sums), reported in the
    results table rather than drawn as per-arrow inference.
    """
    apply_style()
    ax = ax or plt.gca()
